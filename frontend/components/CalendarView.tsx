...
import { GroupTasks } from "@/lib/use-group-tasks";
...
// Month calendar of a group's tasks and milestones. Rendered by the Calendar page and the
// Timeline page's calendar mode; the caller owns the data hook so views share one copy.
export default function CalendarView({ data }: { data: GroupTasks }) {
  const { user } = useAuth();
  const { tasks, milestones, modal, setModal, saving, load, save, remove, reschedule, saveMilestone, deleteMilestone } = data;
  ...
  const chip = (task: Task) => {
    ...
    return <button key={task.id} draggable={!saving} onDragStart={(e) => { e.dataTransfer.setData("text/plain", task.id); e.dataTransfer.effectAllowed = "move"; }}
      ...
      className={`flex w-full items-center gap-1 rounded border px-1.5 py-0.5 text-left text-xs transition duration-150 hover:shadow-sm active:scale-[0.97] ${STATUS_STYLE[s].chip} ${saving ? "cursor-wait" : "cursor-grab"}`}>
